...
- Do not self-review or self-approve — hand off to a separate reviewer if needed.
...
2. Explore the codebase to understand what to document (use `grep`/`glob` and `view` in parallel).
3. Study existing documentation for style, structure, and conventions.
4. Write documentation with verified code examples.
5. Test all commands and examples via `bash`.
6. Report what was documented and verification results.

## Tool Usage

- Use `view` and `grep`/`glob` to explore codebase and existing docs.
- Use `edit` to create or update documentation files.
